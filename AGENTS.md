...
Applicants have no login. Flow: fill form + upload docs (explicit `file_<DocumentType>` multipart keys) → system validates (fields, docs, file format, email dedup, rejects unrequested OTHERS) → atomic save to R2 + DB → returns `TKT-YYYY-NNNNN`. Applicants stored in `users` table as contacts without credentials.
