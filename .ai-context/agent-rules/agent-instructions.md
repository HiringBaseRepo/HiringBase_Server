...
- When editing files under `app/features/**/routers`, `services`, or `repositories`, follow matching path instructions in `./instructions/`.
...
- no direct `BaseDomainException` raises (use subclasses like `ValidationError`, `NotFoundError`, etc.)
- no hardcoded Indonesian strings in app logic (use `get_label` with `ERR_*` or message constants)
- constants and enums have a single source of truth in `app/shared` (avoid duplicates in `settings.py`)
