...
7. **Security & Audit**:
   ...
   - Notification creation and read-state transitions are auditable domain events.
...
10. **Notification Delivery**:
   ...
   - Delivery uses polling REST, not WebSocket.
   - Notification payload stores English/internal keys and params in DB, then resolves Indonesian title/message during response shaping.
   - Notification producers currently live in business events such as public application submit, screening outcomes, interview scheduling, and selected manual status transitions.
