...
def _evaluate_knockout_rule(rule: JobKnockoutRule, application: Application, docs: list) -> bool:
    # Simple deterministic evaluation
    if rule.rule_type == "document":
        doc_types = {d.document_type.value for d in docs}
        return rule.target_value in doc_types
    return True  # default pass for complex rules in MVP
